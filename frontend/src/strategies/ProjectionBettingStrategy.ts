...
import { FeatureManager } from '../unified/FeatureFlags';
import { IntegratedData, Strategy, Decision, Recommendation } from '../core/PredictionEngine';
...
export class ProjectionBettingStrategy implements Strategy {
  ...
  public async analyze(data: IntegratedData): Promise<Decision> {    const traceId = this.performanceMonitor.startTrace('betting-strategy', {
      strategy: this.id,
      timestamp: (((data as Record<string, unknown>).timestamp as number) ?? Date.now()).toFixed(0),
    });
    ...
    try {
      ...
      // Process projections
      const projections = this.processProjections((data as Record<string, unknown>).projections ?? {});
      ...
      const decision: Decision = {
        ...
        recommendations: filteredRecommendations,        analysis: {
          meta_analysis: {
            data_quality: this.calculateDataQuality(data),
            prediction_stability: this.calculatePredictionStability(recommendations),
            market_efficiency: this.calculateMarketEfficiency((data as Record<string, unknown>).odds ?? {}),
            playerId: Object.keys((data as Record<string, unknown>).projections ?? {})[0] || '',
            metric: 'combined',
          },
          confidence_factors: this.calculateConfidenceFactors(data as Record<string, unknown>),
          risk_factors: this.calculateRiskFactors(data as Record<string, unknown>),
        },
      };
  ...
  private calculateMarketEfficiency(odds: Record<string, unknown>): number {
    ...
    const movements = markets.map((m: Record<string, unknown>) => (m.movement as Record<string, unknown>)?.magnitude as number || 0);
  ...
  private calculateProjectionConfidence(projections: unknown): number {
    ...
    const confidences = Object.values(projectionsRecord).map((p: unknown) => (p as Record<string, unknown>).confidence as number || 0);
  ...
  private calculateRiskFactors(data: Record<string, unknown>): Record<string, number> {
    return {
  ...
  private calculateMarketVolatility(odds: unknown): number {
    ...
    return (
      Object.values(oddsRecord)
        .map((m: unknown) => ((m as Record<string, unknown>).movement as Record<string, unknown>)?.magnitude as number || 0)
        .reduce((acc, mag) => acc + mag, 0) / Object.keys(oddsRecord).length || 0
    );
  }
  ...
  private calculateInjuryRisk(injuries: unknown): number {
    ...
    return (
      Object.values(injuriesRecord)
        .map((i: unknown) => (i as Record<string, unknown>).impact as number || 0)
        .reduce((acc, impact) => acc + impact, 0) / Object.keys(injuriesRecord).length || 0
    );
  }
