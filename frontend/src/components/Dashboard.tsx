...
const Dashboard: React.FC = () => {
  ...
  const [riskProfile, _setRiskProfile] = useState<RiskProfileType>(RiskProfileType.MODERATE); // Use enum
  ...
  const [arbitrageOpportunities, _setArbitrageOpportunities] = useState<ArbitrageOpportunityItem[]>([
    {
      id: 'arb1',
      sport: 'Soccer',
  ...
  const [currentPlayerId, _setCurrentPlayerId] = useState<string | null>('player123');
  const [currentMetric, _setCurrentMetric] = useState<string | null>('points');
  const [currentPredictionData, _setCurrentPredictionData] = useState<PredictionData | undefined>({
    value: 25.5,
    confidence: 0.78,
    timestamp: Date.now(),
  });
  ...
  return (
    <div className="flex h-screen bg-gray-100 dark:bg-gray-900">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <Navbar onMenuClick={handleMenuClick} onSmartSidebarClick={handleSmartSidebarClick} />
